...
const apiRequest = async () => {
  ...
  // This endpoint (https://www.fruityvice.com/doc/index.html#api-GET-getAll)
  // returns a list of all the fruits and their info, feel free to play around
  // with different endpoints!
  const resourcePath = 'fruit/all'

  // Making a fetch request to an API endpoint
  // Note: a fetch request is an asynchronous operation, and `await` tells the
  // program to wait until the request has been completed before continuing
...
const updatePage = async () => {
  ...
  const compareFruit = (fruitToCompare) => {
    return fruitToCompare.nutritions.sugar > 15;
  }

  const filteredFruit = fruitsArray.filter(compareFruit);

  console.log(filteredFruit);

  // TODO: Create a new HTML element to display your data
  const fruitList = document.createElement("ul");
  //document.append(fruitList);
  filteredFruit.map((item) => {
    const fruitListItem = document.createElement("li");
    fruitListItem.innerHTML = item.name;
    fruitList.appendChild(fruitListItem);
  })

  // TODO: Append your new element to the page
  document.getElementById("cs1300-gallery").appendChild(fruitList);
